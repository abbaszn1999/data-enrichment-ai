-- Atomic run claim and usage accumulation for Description Visualizer.
...
DECLARE
  claimed_id uuid;
BEGIN
  IF p_phase IS NULL OR p_phase NOT IN ('description', 'images') THEN
    RETURN false;
  END IF;
...
BEGIN
  ...
  IF p_active_phase IS NOT NULL
     AND p_active_phase NOT IN ('description', 'images') THEN
    RETURN false;
  END IF;
...
REVOKE ALL ON FUNCTION public.claim_visualizer_session_run(uuid, uuid, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.add_visualizer_session_usage(
  uuid, uuid, numeric, numeric, integer, integer, text, text, boolean, text
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.claim_visualizer_session_run(uuid, uuid, text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.add_visualizer_session_usage(
  uuid, uuid, numeric, numeric, integer, integer, text, text, boolean, text
) TO service_role;
