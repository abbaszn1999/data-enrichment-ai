...
import type {
  ...
  ProposedCollection,
  StrategyArticle,
} from "@/components/free-assessment/workspace-data";
import {
  projectStateSlice,
  rowsToPersisted,
  type FaProjectRow,
  type FaProjectStateJson,
} from "./project-state";
import {
...
export async function loadFaPersistedState(
  admin: PersistAdmin,
  workspaceId: string
  ...
  const projectRows = (rows ?? []) as FaProjectRow[];
  ...
  await Promise.all(
    projectRows.map(async (row) => {
      ...
      const state = (row.state && typeof row.state === "object" ? row.state : {}) as FaProjectStateJson;
        ...
        (async () => {
          try {
            ...
            if (Array.isArray(data) && data.length > 0) {
              ...
              const legacySample = await loadFaJsonAdmin<ExtractedKeyword[]>(
                admin,
                row.keywords_path
              );
...
export async function saveFaPersistedState(
  admin: PersistAdmin,
  workspaceId: string,
  userId: string,
  ...
  for (const project of persisted.projects) {
    ...
    const existingState =
      existing?.state && typeof existing.state === "object"
        ? (existing.state as FaProjectStateJson)
        : {};
    ...
    const heavySlices: Array<{ name: FaSliceName; payload: unknown; empty: boolean }> = [
      {
        name: "niches",
        payload: {
      ...
      // and `/api/free-assessment/agent/seeds`, never through client autosave —
      // a project's product set can be 20,000+ records, and the client always
      // reports an empty `productsByProject` now (see assessment-shell.tsx).
      {
        name: "seeds",
        payload: {
      ...
      uploads.push(
        saveProjectSliceAdmin(admin, workspaceId, projectId, name, payload).catch(
          (err) => {
            ...
            console.error(`[saveFaPersistedState] Failed to save ${name} slice:`, err);
          }
        )
      );
    }
    ...
    const stateForDb: FaProjectStateJson = {
    ...
    const { error } = existingById.has(projectId)
      ? await admin.from("fa_projects").update(updateFields).eq("id", id)
      : await admin.from("fa_projects").insert({
          ...payload,
          created_by: userId,
        });
    if (error) throw error;
  ...
  const { error: prefsError } = await admin.from("fa_workspace_prefs").upsert({
    workspace_id: workspaceId,
    ...
    await Promise.all(
      orphanIds.map((orphanId) =>
        deleteProjectStorageFolder(admin, workspaceId, orphanId).catch((err) =>
          console.error("[saveFaPersistedState] Cleanup orphan storage failed:", err)
        )
      )
    );
...
export async function createFaProjectRow(
  admin: PersistAdmin,
  input: {
    workspaceId: string;
    userId: string;
  ...
  const { error } = await admin.from("fa_projects").insert({
    id,
  ...
  if (error) throw error;
  await admin.from("fa_workspace_prefs").upsert({
    workspace_id: input.workspaceId,
...
export async function getFaProject(
  admin: PersistAdmin,
  workspaceId: string,
  projectId: string
) {
...
export async function deleteFaProjectRow(
  admin: PersistAdmin,
  workspaceId: string,
  projectId: string
): Promise<boolean> {
