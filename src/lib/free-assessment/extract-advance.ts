...
import type {
  ...
  KeywordIdeasPoll,
  KeywordRow,
} from "@/lib/free-assessment/providers/keyword-provider";
import { nextRowsReturned } from "@/lib/free-assessment/extract-progress";
import {
  ...
  await Promise.all(
    runs.map(async (run) => {
      ...
      if (error && /next_cursor/i.test(error.message)) {
        await admin.from("fa_runs").update({ status: "aborted" }).eq("id", run.id);
      }
    })
  );
...
export async function advanceMrExtract(
  ...
  params: {
    ...
    userId: string;
    provider?: KeywordDataProvider;
    /** Used only when `fa_runs.next_cursor` is empty (migration not applied). */
