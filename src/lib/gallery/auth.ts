...
export async function requireGalleryAuth(options: {
  ...
  if (authError || !user) {
    ...
    if (transient) {
      galleryError("auth", "Supabase auth service is temporarily unreachable", authError);
      return {
        ok: false,
    ...
    galleryWarn("auth", "Not authenticated", {
  ...
  try {
    ...
    galleryError("auth", "Workspace context lookup failed", error);
    return {
      ok: false,
      response: NextResponse.json(
...
export async function requireGalleryAdmin(workspaceId: string, userId: string) {
