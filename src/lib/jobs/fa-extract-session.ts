...
import {
  ...
  seedIsFinished,
  settleHeldExtract,
} from "@/lib/free-assessment/extract-advance";
...
export async function runFaExtractSession(runId: string): Promise<void> {
  await runJobWithFailureGuard(runId, () => runFaExtractSessionInner(runId));
}
...
async function runFaExtractSessionInner(runId: string): Promise<void> {
  const admin = createAdminClient();
  const job = await loadJobRun(admin, runId);
  if (!job || job.kind !== "fa_extract") return;
  ...
  await dispatchJob(job.id, "fa_extract");
}

export const FA_EXTRACT_TUNING = {
  PUMP_BUDGET_MS,
  TICK_MS,
  EXTRACT_MAX_AGE_MS,
} as const;
