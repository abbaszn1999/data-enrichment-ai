...
export function useFaWallet(workspaceId: string | null | undefined) {
  const walletVersion = useWorkspaceStore((s) => s.faWalletVersion);
...
export async function topUpFaWalletApi(
  workspaceId: string,
  amountUsd: number,
  method: string
): Promise<WalletState> {
  const response = await fetch("/api/free-assessment/wallet/topup", {
    method: "POST",
...
export async function startFaWalletCheckoutApi(
  workspaceId: string,
  workspaceSlug: string,
  amountUsd: number
): Promise<string> {
  const response = await fetch("/api/free-assessment/wallet/checkout", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function saveFaWalletAutoReloadApi(
  workspaceId: string,
  autoReload: WalletAutoReload
): Promise<WalletState> {
  const response = await fetch("/api/free-assessment/wallet/settings", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
