...
export interface ImportStep {
  num: number;
  label: string;
  /** Route segment under /catalog-intelligence/[sessionId]; the workspace step is the session root. */
  segment: string | null;
}
...
export function getImportSteps(kind: SessionKind = "product"): ImportStep[] {
  if (kind === "plp") {
    ...
    return [
      ...
      { num: 3, label: "PLP Workspace", segment: null },
    ];
  }
  return [
    { num: 1, label: "Upload File", segment: null },
    ...
    { num: 4, label: "Enrichment Tool", segment: null },
  ];
}
