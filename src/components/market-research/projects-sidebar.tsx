...
function ProjectAvatar({
  ...
  size?: "sm" | "md";
  /** Active project stays vivid; others look extinguished. */
  lit?: boolean;
}) {
  ...
  return (
    <div
      ...
      aria-hidden
    >
      <span className="relative flex h-2.5 w-4 items-center justify-center gap-1">
...
/**
 * Collapsible projects rail with per-project ⋮ menu (rename / delete).
 */
export function ProjectsSidebar({
  ...
  const [deleteTarget, setDeleteTarget] = useState<MarketResearchProject | null>(null);
  ...
  const beginRename = (project: MarketResearchProject) => {
  ...
  return (
    <>
      <aside
        className={`relative flex h-full min-h-0 shrink-0 flex-col overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm transition-[width] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          open ? "w-[260px]" : "w-[56px]"
        }`}
      >
        {/* Collapsed rail */}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleteTarget
                ? `“${deleteTarget.name}” will be removed from this workspace preview. This cannot be undone.`
