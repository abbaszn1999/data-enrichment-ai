...
export type MarketResearchPersisted = {
  ...
  /** Non-taxonomic PLPs the Stage 1 taxonomy agent excluded (visible, never
   *  selectable, zero SKUs), per project. */
  taxonomyExcludedByProject: Record<string, MockExcludedItem[]>;
  /**
   * Minimum SKUs a category/subcategory/PLP needs to be selectable in Tab 2
   * — a persistent per-project merchant setting (default 100), promoted
   * from a client-only QA test control.
   */
  skuFloorByProject: Record<string, number>;
...
export function loadMarketResearchState(
  ...
  try {
    ...
    return {
      ...
      taxonomyExcludedByProject: parsed.taxonomyExcludedByProject ?? {},
      skuFloorByProject: isNumberMap(parsed.skuFloorByProject),
