...
export function StageCollectionSheet({
  ...
  walletHref?: string;
  pushing?: boolean;
  /** Free assessment stops at the collection list — no SKU preview or store push. */
  showStoreActions?: boolean;
  /**
   * Stage 5 Phase 3 flagged these ids' shopper-intent as already covered by
   * an existing PLP on the merchant's uploaded sheet (`status === "duplicate"`).
   * Passing this permanently removes them from the collections list and from
   * the current selection — a one-time bulk action, not a toggle/filter,
   * triggered manually by the merchant via the "Remove Duplicates" button.
   */
        ...
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            ...
            {showStoreActions
              ...
              : " · SKU matching and store push are off on this assessment."}
          </p>
        </div>
