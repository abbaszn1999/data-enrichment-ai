...
/** Frontend stand-in until the assessment extract backend is wired: one row per seed term. */
export function keywordsFromSeeds(
  seeds: MockSeedRow[],
  probes: Record<string, SeedProbe>
): ExtractedKeyword[] {
  return seeds.map((seed, index) => {
    const probe = probes[seed.id];
    const keyword = seed.broadSeedVariation;
    const volume = probe && !probe.failed ? probe.searchVolume : 100 + (index % 80) * 12;
    return {
      id: `${seed.id}-kw`,
      seedId: seed.id,
      seed: keyword,
