...
export type MarketResearchPersisted = {
   ...
   * — a persistent per-project setting, promoted from a client-only QA test
   * control.
   */
...
function storageKey(workspaceSlug: string) {
  return `free-assessment:v1:${workspaceSlug}`;
}

function migratedKey(workspaceSlug: string) {
  return `free-assessment:migrated:v1:${workspaceSlug}`;
}
