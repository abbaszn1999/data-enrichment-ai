...
import { creditFaWallet, readFaWallet } from "@/lib/free-assessment/wallet-server";
...
export async function POST(request: NextRequest) {
  ...
  const credited = await creditFaWallet(auth.admin, {
    workspaceId: parsed.data.workspaceId,
    userId: auth.user.id,
    amountUsd: parsed.data.amountUsd,
    kind: "topup",
    description: "Free Assessment wallet top-up",
    module: "Billing",
    method: parsed.data.method ?? "Dev credit",
    details: { source: "fa_dev_topup" },
  });
