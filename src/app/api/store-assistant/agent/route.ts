...
export async function POST(request: NextRequest) {
  ...
  if (resumeCheckpoint) {
    const checkpoint = await loadStoreAssistantCheckpoint(admin, workspaceId);
    if (checkpoint?.sheet) sheet = checkpoint.sheet;
  }
  ...
  const stream = createNdjsonStream(async (push) => {
    ...
    const onLoopEvent = (event: AgentLoopEvent) => {
      switch (event.type) {
        ...
        case "final": {
          void clearStoreAssistantCheckpoint(admin, workspaceId).catch(() => undefined);
          push({
            type: "result",
            data: {
